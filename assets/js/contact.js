...
$("#guardarAdjuntos").on('click', function (e) {
    $('#guardarAdjuntos').prop("disabled", true);
    var error = false;
    var data = new FormData();

    if (fileArreglo.length) {
        $.each(fileArreglo, function (index, file) {
            data.append("files[]", file);
        });
    } else {
        alert('Suba al menos un archivo.', 'Adjuntar archivos');
    }
    if (!error) {
        data.append("id_paciente", atencion.id_paciente);
        uploadFiles(data);
    } else
        $('#guardarAdjuntos').prop("disabled", false);
});


function uploadFiles(data) {
    $.ajax({
        url: '127.0.0.1:8080/historialadjuntopersona',
        type: "POST",
        data: data,
        processData: false,
        contentType: false,
